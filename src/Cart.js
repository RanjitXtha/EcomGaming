    ...
    <div style={{margin:'10rem 15rem'}}>
      ...
      {cartItems.map((item, index) => (
        <div className="cart-product" key={index}>
          ...
          <button style={{fontSize:'100%',marginTop:'0px'}} className="buy-now-button" onClick={() => removeItem(index)}>Remove</button>
        </div>
      ))}
      <h2 style={{marginTop:'5rem'}}>Price: $ {totalCost}</h2>
      <button style ={{width:'13rem',marginTop:'5rem',fontSize:'100%'}} className="buy-now-button" onClick={clearCart}>Clear Cart</button>
    </div>
  );
};
